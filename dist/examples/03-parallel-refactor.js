...
async function runParallelRefactor() {
    ...
    // Create workers
    const researchWorker = new ResearchWorker_1.ResearchWorker({ id: 'researcher', type: 'research', maxConcurrentTasks: 5, timeoutMs: 10000 }, messageBus);
    ...
    for (let i = 1; i <= 3; i++) {
        new ImplementWorker_1.ImplementWorker({ id: `implementer-${i}`, type: 'implement', maxConcurrentTasks: 2, timeoutMs: 15000 }, messageBus);
        coordinator.registerWorker({ id: `implementer-${i}`, type: 'implement', maxConcurrentTasks: 2, timeoutMs: 15000 });
    }
    const verifyWorker = new VerifyWorker_1.VerifyWorker({ id: 'verifier', type: 'verify', maxConcurrentTasks: 5, timeoutMs: 20000 }, messageBus);
    ...
    console.log('═══════════════════════════════════════════\n');
    const researchTasks = filesToRefactor.map((file, index) => ({
        id: `research-${index}`,
        type: 'research',
    ...
    const implementTasks = chunks.map((chunk, index) => ({
    ...
    const passedChecks = allChecks.filter((c) => c.success).length;
...
//# sourceMappingURL=03-parallel-refactor.js.map