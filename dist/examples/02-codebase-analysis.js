...
async function runCodebaseAnalysis() {
    ...
    const workers = [
        { id: 'analyzer-1', type: 'research' },
        { id: 'analyzer-2', type: 'research' },
        { id: 'analyzer-3', type: 'research' },
        { id: 'analyzer-4', type: 'research' }
    ];
    workers.forEach(w => {
        new ResearchWorker_1.ResearchWorker({ id: w.id, type: w.type, maxConcurrentTasks: 3, timeoutMs: 15000 }, messageBus);
    ...
    const aggregated = analysisResults.results.reduce((acc, result) => {
        if (result.data?.files) {
            ...
            acc.totalComplexity += result.data.analysis?.reduce((sum, a) => sum + a.complexity, 0) || 0;
        }
        if (result.data?.direct) {
            acc.dependencies = result.data;
        }
        return acc;
    }, { totalFiles: 0, totalComplexity: 0, dependencies: {} });
    console.log('═══════════════════════════════════════════');
...
//# sourceMappingURL=02-codebase-analysis.js.map